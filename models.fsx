...
module JsonParser =
    ...
    let parseModel (id: string) (element: JsonElement) : ModelInfo option =
        match getStringValue element "name", getStringValue element "family",
              parseCost element, parseLimit element, parseModalities element with
            ...
            let models =
                ...
                | Some modelsEl ->
                    if modelsEl.ValueKind = JsonValueKind.Object then
                        ...
                        |> Seq.choose (fun prop ->
                            parseModel prop.Name prop.Value)
                        |> List.ofSeq
                    else
                        []
                | None -> []
    ...
    let parseModelsJson (jsonPath: string) : ProviderModels list =
        try
            ...
            |> Seq.choose (fun prop ->
                parseProvider prop.Name prop.Value)
            |> List.ofSeq
        with
        | ex ->
...
module FuzzyMatch =
    ...
    let calculateSimilarity (s1: string) (s2: string) : float =
        ...
        else 1.0 - float distance / float maxLen
...
module ModelQuery =
    ...
    let queryModelByIdStartOrEnd (providers: ProviderModels list) (modelId: string) : ModelInfo option =
        providers
        |> List.collect (fun p -> p.models)
        |> List.tryFind (fun m -> m.id.StartsWith(modelId, StringComparison.OrdinalIgnoreCase) || m.id.EndsWith(modelId, StringComparison.OrdinalIgnoreCase))
    ...
    /// 获取模型的成本信息
    let getCost (model: ModelInfo) : Cost =
        model.cost

    /// 获取模型的限制信息
    let getLimit (model: ModelInfo) : Limit =
        model.limit

    /// 根据模型查询输入成本（每1k tokens）
    let getInputCost (model: ModelInfo) : float =
        model.cost.input

    /// 根据模型查询输出成本（每1k tokens）
    let getOutputCost (model: ModelInfo) : float =
        model.cost.output

    /// 获取上下文长度（tokens）
    let getContextLength (model: ModelInfo) : int =
        model.limit.context

    /// 获取输出长度限制（tokens）
    let getOutputLength (model: ModelInfo) : int =
        model.limit.output
    ...
    /// 查询模型是否支持reasoning
    let supportsReasoning (model: ModelInfo) : bool =
        model.reasoning

    /// 查询模型是否支持tool calling
    let supportsToolCall (model: ModelInfo) : bool =
        model.tool_call
...
let getModelCost (modelQuery: string) (threshold: float) : (float * float) option =
    ...
    | Some model ->
        Some (model.cost.input, model.cost.output)
    | None -> None
...
let getModelLimit (modelQuery: string) (threshold: float) : (int * int) option =
    match queryModel modelQuery threshold with
    | Some model ->
        Some (model.limit.context, model.limit.output)
    | None -> None
...
let calculateCallCost (modelQuery: string) (threshold: float) (inputTokens: int) (outputTokens: int) : float option =
    match queryModel modelQuery threshold with
    | Some model ->
        Some (ModelQuery.calculateCost model inputTokens outputTokens)
    | None -> None
